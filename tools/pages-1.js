   ...
   ================================================================== */
{
  ...
  crumb: "はじめに",
  recap: [
    "この14本は、<b>ブラウザだけ</b>で動きます。インストールは何も要りません。",
  ...
  html: `
    ...
    <ol class="steps">
      ...
      <li>
        ...
        <p>最初の1本は <b>トースト.js</b> がおすすめです。14本のなかでいちばん構造が素直で、
    ...
    <p>
      ...
      汁物（お味噌汁）、甘いもの（クッキー・チョコレート・団子）、そして寿司。
  ...
  tryIt: [
    ...
    "14本のファイルの先頭のコメントだけを読み比べて、共通する見出しを探す。"
  ]
},
   ...
   ================================================================== */
{
  ...
  html: `
    ...
    <ul>
      ...
      <li><b>軽い</b> — 14本ぜんぶ合わせて 1.5MB ほどのテキストだけです。
...
//    9. Scene / GUI</span></pre>
      <figcaption>たこ焼き.js の冒頭。料理名のところが入れ替わるだけで、14本ともこの並び</figcaption>
    </figure>
    <p>
    ...
    <p>
      ...
      14本のうち9本は、焼いたり揚げたりしたものです。その9本には例外なく
  ...
  tryIt: [
    ...
    "14本のファイルを検索して、bake / heat / char という語がどのファイルに出てくるか調べ、「焼いた料理」と一致するか確かめる。",
   ...
   ================================================================== */
{
  ...
  crumb: "第02章",
  recap: [
    "全14本とも <code>Math.random()</code> を使っていません。代わりに<b>自分で乱数を書いて</b>います。",
...
<span class="k">const</span> Rng = (seed) =&gt; {
    ...
    <span class="k">return</span> () =&gt; {
      ...
      <figcaption>14本すべてに、ほぼこの形で入っている。名前は mulberry32</figcaption>
    </figure>
    <p>
...
<span class="k">const</span> rndB = Rng(seed + i * <span class="n">1013</span> + <span class="n">77</span>); <span class="c">// 焼き色</span></pre>
      <figcaption>種に「ずらす数」を足して枝分かれさせる。14本に共通のやり方</figcaption>
    </figure>
    <div class="trap">
...
<span class="k">const</span> v2 = (x, y, seed) =&gt; {
    ...
    <span class="k">return</span> mix(mix(a, b, u), mix(c, d, u), v);
      ...
      <figcaption>14本すべてに、ほぼこの形で入っている</figcaption>
    </figure>
...
<pre><span class="k">const</span> fbm = (x, y, seed, oct) =&gt; {
    ...
    <p>
      ...
      14本のうち筒状のものには、必ずこの <code>v2u</code> が入っています。
    </p>
    <div class="trap">
      <span class="label">つまずきポイント</span>
    ...
    <div class="lab" id="lab04">
      ...
      <div class="lab-cols two">
        ...
        <div>
          ...
          <p class="hint">その断面を1周ぶん回した結果。ドラッグで回せます。</p>
        </div>
      </div>
...
<span class="k">for</span> (<span class="k">let</span> i = <span class="n">0</span>; i &lt; N; i++) {
    ...
    <p>
      14本のサンプルは、この間違いを<b>実行時に自分で検出して直します</b>。
...
<span class="k">if</span> (dot &lt; <span class="n">0</span>) flipIndices(indices);</pre>
    ...
    <div class="lab" id="lab05">
      ...
      <p class="hint">
        ...
        ふつうに描いたときは中身が透けて見えます。<br><b>立体はドラッグで回せます</b>（触ると自動回転は止まります。ダブルクリックで戻ります）。
        上から覗くと、裏返した面がどこにあるかがよく分かります。
      </p>
    </div>
    ...
    <p>
      ...
      14本すべてに <code>weldNormals</code> という関数が入っているのはそのためです。
    </p>
    <div class="trap">
      <span class="label">つまずきポイント</span>
...
<span class="k">for</span> (<span class="k">let</span> y = <span class="n">0</span>; y &lt; S; y++) <span class="k">for</span> (<span class="k">let</span> x = <span class="n">0</span>; x &lt; S; x++) {
    ...
    <div class="trap">
      ...
      Babylon.js は V が下向きなので前者。14本のうち5本のコメントに、
   ...
   ================================================================== */
{
  ...
  html: `
    ...
    <table>
      <tr><th>数値</th><th>意味</th><th>この14本での使いどころ</th></tr>
    ...
    <div class="lab" id="lab07">
      ...
      <p class="hint">
        ...
        食べ物の艶は「鋭い」のではなく「広くにじむ」ものです。<br><b>立体はドラッグで回せます</b>（触ると自動回転は止まります。ダブルクリックで戻ります）。
        艶の見え方は角度でまるで変わるので、回しながら決めてください。
      </p>
    </div>
   ...
   ================================================================== */
{
  ...
  recap: [
    ...
    "ただし14本のうち何本かは、あえて物理を使いません。<b>転がってはいけない料理</b>があるからです。"
  ],
  html: `
    <p>
...
flake.thinInstanceSetBuffer(<span class="s">"color"</span>, colors, <span class="n">4</span>);</pre>
    ...
    <div class="trap">
      ...
      14本のサンプルは「name を変えたら id も必ず併せて変える」を規約にしました。
...
<span class="k">for</span> (<span class="k">let</span> i = <span class="n">0</span>; i &lt;= NS; i++) {
    ...
    <div class="lab" id="lab10">
      ...
      <p class="hint">
        ...
        秋刀魚は断面のひらたさを 0.6 くらいにすると、それらしくなります。<br><b>立体はドラッグで回せます</b>（触ると自動回転は止まります。ダブルクリックで戻ります）。
        真上から見ると、軸の曲がりがそのまま輪郭に出ているのが分かります。
      </p>
    </div>
  ...
  html: `
    ...
    <div class="lab" id="lab11">
      ...
      <p class="hint">
        ...
        入れておくと、白くなる手前で色が残り続けます。<br><b>立体はドラッグで回せます</b>（触ると自動回転は止まります。ダブルクリックで戻ります）。
        光源は動かないので、回すと「同じ光でも角度で別物になる」ことが見えます。
      </p>
    </div>
    ...
    <p>
      ...
      14本のサンプルは最初、これを避けるために<b>GUI 専用のカメラをもう1台立てて</b>、
