// =====================================================================
//  Photoreal Bowl of Rice  /  写実的な「ごはん」（茶碗一杯）  BUILD: rice-B
//  Babylon.js Playground 用（そのまま貼り付けて実行できます）
//
//  rice-A からの変更点（Inspector のデバッグ表示）:
...
//      選ぶ」問題は、このシーンには無い。粒は変種が grain0..grainN、
//      その複製が g0..gN と通し番号で作られており、器・ごはん本体・
//      テーブルも1つずつなので、名前も id もすべて一意になっている。
//      ただし Babylon の Node はコンストラクタで id = name を入れるので、
//      今後 同名のメッシュを増やすときは name と id の両方を必ず
